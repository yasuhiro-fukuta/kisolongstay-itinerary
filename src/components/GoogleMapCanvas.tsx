...
export default function GoogleMapCanvas({
  ...
  useEffect(() => {
    ...
    places.textSearch({ query: q }, (results, status) => {
      ...
      const mapUrl = placeId ? `https://www.google.com/maps/place/?q=place_id:${placeId}` : undefined;
  ...
  useEffect(() => {
    ...
    return () => {
      // do nothing (abort handled per day)
    };
  }, [routePlans]);
